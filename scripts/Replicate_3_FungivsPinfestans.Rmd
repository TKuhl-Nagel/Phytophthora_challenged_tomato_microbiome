...
#Read data
Rep3_withzeros <- read_excel("~/Ad_Planta_Trials/Raw_data/Rep_three.xlsx")
#rename column Diseased_leaf_area
colnames(Rep3_withzeros)[colnames(Rep3_withzeros) == "Diseased_leaf_area"] <- "Diseased leaf area (%)"
View (Rep3_withzeros)


#rename columns
library(dplyr)
Rep3_withzeros$Treatment <- dplyr::recode(Rep3_withzeros$Treatment,
   "Pf23"="Pseudogymnoascus sp._Pf23",
    "Pf101" = "Chaetomium sp._Pf101", 
    "Pf27" = "Chlonostachys sp._Pf27",
    "Cu" = "Copper standard",
    "Pf45" = "Ctenomyces sp._Pf45",
    "Pc" = "Positive control",
    "Nc" = "Negative control",
    "Pf131" = "Trichoderma sp._Pf131")
View (Rep3_withzeros)

#reorder
Rep3_withzeros$Treatment <- factor(
  Rep3_withzeros$Treatment,
  levels = c(
    "Positive control",
    "Negative control",
    "Copper standard",
...
Rep3_withzeros$`Diseased leaf area (%)` <- as.numeric(Rep3_withzeros$`Diseased leaf area (%)`)


#List rows with Diseased leaf area of 0
Rep3_withzeros[Rep1_withzeros$`Diseased leaf area (%)` == "0", ]
#remove them
Rep3<- Rep3_withzeros[Rep1_withzeros$`Diseased leaf area (%)` != 0, ]
```
```{r}
...
Rep3_withzeros_mean <- Rep3_withzeros %>%
  group_by(`Treatment`, Plant) %>% summarize_at("Diseased leaf area (%)", mean)
...
ggplot(Rep3_mean, aes(sample = `Diseased leaf area (%)`)) +
...
colnames(Rep3_mean)[3] <- 'Diseased_leaf_area'


two_way_anova_Rep3 <- aov(Diseased_leaf_area ~ Treatment, data = Rep1_mean)
summary(two_way_anova_Rep3)


# Perform Tukey test Area of infection
#this test checks for which treatments are significantly different from each other
tukey_results_Rep3 <- TukeyHSD(two_way_anova_Rep3)

plot(tukey_results_Rep3, las = 1)
...
cld <- multcompLetters4(two_way_anova_Rep3, tukey_results_Rep3)
```
```{r}
...
merge(x = Rep3_withzeros_mean, y = df, by = "Treatment", all.y = TRUE)
...
# Plot
FungivsPinfestans_Rep3 <- ggplot(Rep3_withzeros_mean, aes(x = Treatment, y = `Diseased leaf area (%)`, fill = Treatment)) +
...
# Save plot as TIFF
ggsave(
  filename = "Fungi_Rep3.tiff",
  plot = FungivsPinfestans_Rep3,
