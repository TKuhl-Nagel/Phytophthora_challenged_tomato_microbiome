...
Read data
```{r}
ASVs <- read.table("~/Philemon_ITS/tables_for_analysis/Megablast_Philemon_dada2_Unite_final.txt", header=T, sep="\t", dec=".", quote="", fill=FALSE) #add manually in textfile a 0 in front of single numbers 1, 2, 3, 4 ,5 ...
head(ASVs)
dim(ASVs)

tax <- read.table("~/Philemon_ITS/tables_for_analysis/Megablast_Philemon_dada2_Unite_taxonomy.txt", header=TRUE, sep="\t") 
head(tax)
```
...
#Export
#write.table(ASVs_no0, "~/Philemon_ITS/tables_for_analysis/Megablast_Philemon_dada2_UNITE_no0_ASVNumbers2.txt", dec = ".", sep = "\t")
...
#Export
#write.table(tax_final, "~/Philemon_ITS/tables_for_analysis/Megablast_Philemon_dada2_Unite_taxonomy_CLEAN.txt", dec = ".", sep = "\t")
...
Read data
```{r}
ASVs_no0_new <- read.table("~/Philemon_ITS/tables_for_analysis/Megablast_Philemon_dada2_UNITE_no0_ASVNumbers2.txt", header=T, sep="\t", dec=".", fill=FALSE) 
head(ASVs_no0_new)
dim(ASVs_no0_new)
```
...
write.table(mean.dat, file="~/Philemon_ITS/ASV_ITS_Rarefied_Table_1000_times_clean_ASVnumbers.txt", sep = "\t")
