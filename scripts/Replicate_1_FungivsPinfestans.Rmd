...
#Read data
Rep1_withzeros <- read_excel("~/Ad_Planta_Trials/Raw_data/Rep_one.xlsx")
View (Rep1_withzeros)
#rename columns
library(dplyr)
Rep1_withzeros$Treatment <- dplyr::recode(Rep1_withzeros$Treatment,
...
#reorder
Rep1_withzeros$Treatment <- factor(
  Rep1_withzeros$Treatment,
  levels = c(
    "Positive control",
    "Negative control",
    "Copper standard",
...
Rep1_withzeros$`Diseased leaf area (%)` <- as.numeric(Rep1_withzeros$`Diseased leaf area (%)`)

colnames(Rep1_withzeros)

#List rows with Diseased leaf area of 0
Rep1_withzeros[Rep1_withzeros$`Diseased leaf area (%)` == "0", ]
#remove them
Rep1<- Rep1_withzeros[Rep1_withzeros$`Diseased leaf area (%)` != 0, ]
```
```{r}
...
Rep1_withzeros_mean <- Rep1_withzeros %>%
  group_by(`Treatment`, Plant) %>% summarize_at("Diseased leaf area (%)", mean)
...
Rep1_mean %>%
  group_by(Treatment) %>%
  summarise(mean=mean(`Diseased leaf area (%)`, na.rm = TRUE),
            ...
            se=se(`Diseased leaf area (%)`, na.rm = TRUE),
           Plant =n())
```
...
ggplot(Rep1_mean, aes(sample = `Diseased leaf area (%)`)) +
...
colnames(Rep1_mean)[3] <- 'Diseased_leaf_area'


two_way_anova_Rep1 <- aov(Diseased_leaf_area ~ Treatment, data = Rep1_mean)
summary(two_way_anova_Rep1)


# Perform Tukey test Area of infection
#this test checks for which treatments are significantly different from each other
tukey_results_Rep1 <- TukeyHSD(two_way_anova_Rep1)

plot(tukey_results_Rep1, las = 1)
...
cld <- multcompLetters4(two_way_anova_Rep1, tukey_results_Rep1)
```
```{r}
...
merge(x = Rep1_withzeros_mean, y = df, by = "Treatment", all.y = TRUE)
...
# Plot
FungivsPinfestans_Rep1 <- ggplot(Rep1_withzeros_mean, aes(x = Treatment, y = `Diseased leaf area (%)`, fill = Treatment)) +
...
# Save plot as TIFF
ggsave(
  filename = "Fungi_Rep1.tiff",
  plot = FungivsPinfestans_Rep1,
